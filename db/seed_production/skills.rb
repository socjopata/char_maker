...
#Wymagania: Podstawy Akrobatyki, Zręczność 25+



skill = Skill.create(:name => "Czytanie i Pisanie", :way_it_works => "umiejętność czytania i pisania w wybranym języku (najczęściej będzie to język ojczysty postaci), w niektórych przypadkach MG może nie zgodzić się na wyuczenie danego języka z powodu braku odpowiedniego nauczyciela, np.: postać chce poznać język z odległych archipelagów czy zakazany dialekt demonów.
",:active => false )
#Wymagania: Inteligencja 14+



skill = Skill.create(:name => "Dworskość", :description => "jest to zbiór umiejętności potrzebnych każdej kulturalnej osobie, zwłaszcza o pochodzeniu szlacheckim, która w elitarnym towarzystwie nie chce wyjść na totalnego ignoranta i chama. Umożliwia nie tylko odpowiednie zachowanie w towarzystwie, przy stole, na balach i innych spotkaniach ale również pozwala kulturalnie zabawić damę.",
...
#Wymagania: Ogłada i Inteligencja 14+



skill = Skill.create(:name => "Dykcja i Przemawianie", :way_it_works => "Podczas rozmowy, handlowania, wygłaszania mów, itp. działaniach postać zwiększa swą szansę w teście Ogłady o premię z Inteligencji. Ponadto Ogłada bohatera zwiększana jest o +2pkt.",
...
s_choice.stats_modifiers.create(:modifies => "O", :value => 2, :group_name => "domyślne")
#Wymagania: Inteligencja 17+


skill = Skill.create(:name => "Heraldyka i Historia", :way_it_works => "Udany test na Inteligencje pozwala określić historie kryjącą się za danym herbem lub rodem szlacheckim, przypomnieć minione wydarzenia i losy zapisane w starożytnych księgach. Ponadto postać zwiększa swoją Inteligencję o +1pkt.",
...
skill = Skill.create(:name => "Mistrzowska Oburęczność", :way_it_works => "walcząc oburącz postać nie otrzymuje kary -5pkt do Trafienia wykonując dodatkowy atak drugą bronią. Zręczność bohatera podnoszona jest o +1pkt.",
:active => false)
s_choice = skill.stats_choices.create

s_choice.stats_modifiers.create(:modifies => "ZR", :value => 1, :group_name => "domyślne")
#Wymagania: Oburęczność, Siła 15+, Zręczność 25+, Poziom Doświadczenia 3+


skill = Skill.create(:name => "Nauczyciel Fechtunku Wielu Broni", :way_it_works => "bohater zyskuje Jedną Biegłość w Broniach, dzięki czemu będzie mógł nauczyć się walczyć dodatkową grupą broni.",
:active => false)
s_choice = skill.stats_choices.create

#TODO modifiers here

skill = Skill.create(:name => "Nauczyciel Fechtunku Jednej Broni", :way_it_works => "bohater zyskuje w wybranej Grupie Broni +1pkt Fechtunku (dzięki czemu zwiększa zarówno FwA jak i FwO). Dotyczy to każdej broni zarówno tej do walki wręcz jak i strzeleckiej.",
:active => false)
s_choice = skill.stats_choices.create
#TODO modifiers here


skill = Skill.create(:name => "Nieustraszony", :way_it_works => "postać zwiększa swoją Wiarę o +1pkt oraz Odporność na Strach o +5pkt. Jeżeli efekt magiczny skierowany w bohatera wywołuje Strach lub Przerażenie premia ta przyznawana jest również w przeciwstawnym teście Przełamania. Zdolność może być wybrana wielokrotnie, analogicznie podnosząc parametr główny jak i samą odporność.",
:active => false)
...
#Wymagania: Wiara 17+, Wytrzymałość 25+

skill = Skill.create(:name => "Oburęczność", :way_it_works => "postać udoskonaliła technikę walki dwoma orężami, dzięki czemu zyskuje jeden atak z dodatkowej broni z karą do Trafienia -5pkt. Wyliczając swą Obronę korzysta z obu parametrów broni.",
:active => false)
#TODO remember to do something when the total defence is being count
#Wymagania: Siła 13+, Zręczność 20+

skill = Skill.create(:name => "Odporność", :way_it_works => "postać może dwukrotnie więcej przebiec, przepłynąć, przejść marszem, itp. Dodatkowo dwukrotnie wolniej męczy się w walce a jej Odporność na Ból jak i „Punkty Życia” podnoszone są o +3pkt.",
:active => false)

s_choice = skill.stats_choices.create
s_choice.stats_modifiers.create(:modifies => "auxiliary", :value => 3, :group_name => "Odporność na Ból")  #TODO unify the name
s_choice.stats_modifiers.create(:modifies => "auxiliary", :value => 3, :group_name => "Punkty Życia")  #TODO unify the name
#Wymagania: Wytrzymałość 22+


skill = Skill.create(:name => "Pływanie i Nurkowanie", :way_it_works => "postać nauczyła się pływać i nurkować. Jakiekolwiek testy sprawdzające utrzymanie się jej na powierzchni wody, itp. wykonywane są tylko w wyjątkowo trudnych warunkach takich jak: sztorm, duże obciążenie (zbroja) lub poważne rany, testem na Siłę postaci. Postać zyskuje +1pkt do Wytrzymałości.",
:active => false)
s_choice = skill.stats_choices.create
...
skill = Skill.create(:name => "Refleks", :way_it_works => "dobycie broni z pochwy lub pasa nie zabiera punktów Tempa (normalnie akcja ta kosztuje 1pkt Tempa), a Inicjatywa jak i Zręczność bohatera wzrastają o +1pkt.",
:active => false)
...
skill = Skill.create(:name => "Rzemiosło rozwijające siłę", :way_it_works => "postać wyuczyła się wybranego fachu lub rzemiosła. Po kilku latach praktyki, zebrała nieco oszczędności (5k20+100zł), a jej Siła lub Inteligencja podnoszone są o +1pkt. Gracz wraz z MG wybiera rodzaj zawodu jakim para się postać.",
:active => false)
...
skill = Skill.create(:name => "Rzemiosło rozwijające intelekt", :way_it_works => "postać wyuczyła się wybranego fachu lub rzemiosła. Po kilku latach praktyki, zebrała nieco oszczędności (5k20+100zł), a jej Siła lub Inteligencja podnoszone są o +1pkt. Gracz wraz z MG wybiera rodzaj zawodu jakim para się postać.",
:active => false)
...
skill = Skill.create(:name => "Sokoli Wzrok", :way_it_works => "Spostrzegawczość postaci zwiększana jest o +5pkt, ponadto może wykonać dwa rzuty sprawdzając ten parametr i wybrać korzystniejszy wynik.",
:active => false)
...
s_choice.stats_modifiers.create(:modifies => "ZR", :value => 1, :group_name => "domyślne")
#Wymagania: Jeździectwo, Podstawy Akrobatyki, Zręczność 19+

#TODO limitation
skill = Skill.create(:name => "Wykształcenie Akademickie", :way_it_works => "postać zdobyła ogólne wykształcenie, dzięki któremu zna podstawy wszystkich dziedzin nauki. Ponadto zwiększa swój Spryt o +2pkt a wszystkie testy (nie magiczne) związane z wiedzą ułatwione są o 5pkt. Inteligencja postaci wzrasta o +1pkt.",
...
skill = Skill.create(:name => "Wyrób Trucizn", :way_it_works => "patrz rozdział Alchemia. Bohater może wytwarzać tylko takie trucizny, których poznał dokładną recepturę. Postać rozpoczyna grę posiadając pięć wybranych przepisów. Ponadto bohater staje się wyczulony na trucizny, dzięki czemu ma większą szansę wykryć je w jedzeniu czy piciu (test łatwiejszy o +5pkt). Inteligencja truciciela wzrasta o +1pkt.",
:active => false )
s_choice = skill.stats_choices.create

s_choice.stats_modifiers.create(:modifies => "INT", :value => 1, :group_name => "domyślne")
#Wymagania: Zielarstwo, Inteligencja 21+


skill = Skill.create(:name => "Zabijaka", :way_it_works => "wiele lat spędzonych na ulicy, w portach i knajpach, nie tylko zahartowało postać ale również udoskonaliło jej techniki walki. Bohater otrzymuje +2pkt do „Fechtunku” w Bijatyce oraz podnosi swą Siłę jak i Odporność na Ból o +2pkt.",
:active => false )
s_choice = skill.stats_choices.create
...
skill = Skill.create(:name => "Zielarstwo", :way_it_works => "postać posiada rozległą wiedzę dotyczącą roślin, dzięki czemu wie gdzie i co występuje, jak należy to zbierać, suszyć, preparować, itd. Umiejętność ta daje również możliwość wykonywania lub przyrządzania Prostych mikstur, leków bądź eliksirów. Poprawne wykonanie preparatu określone jest zawsze w jego przepisie. Postać zyskuje +1pkt Inteligencji.",
:active => false )
s_choice = skill.stats_choices.create

s_choice.stats_modifiers.create(:modifies => "INT", :value => 1, :group_name => "domyślne")

skill = Skill.create(:name => "Znajomość Regionu Pochodzenia", :way_it_works => "umiejętność zapewnia znajomość wybranego rejonu geograficznego, dróg, ewentualnych niebezpieczeństw: lawiny, rwące rzeki, mielizny, rafy, itp., topografii oraz obyczajności tubylców.",
:active => false )

skill = Skill.create(:name => "Żeglarstwo i Nawigacja", :way_it_works => "Bohater potrafi określić pozycję/kierunek oraz wyznaczyć kurs za pomocą gwiazd. Testy zdolności wykonujemy tylko i wyłącznie w wyjątkowo ciężkich warunkach tj.: sztorm, burza, mgła bądź silne opady, sprawdzając Inteligencję postaci. Inteligencja postaci podnoszona jest o +1pkt.",
