...
class Wizard
  ...
  STEP_NUMBERS_MAP = { 'profession_and_origin' => 1,
  ...
  def initialize(character, step, params = nil)
    ...
    @step = step
    @params = params
    @step_number = STEP_NUMBERS_MAP[step]
  ...
  def statistics
    if params
      if @character.statistics.update_attributes(params[:statistics]) && @character.valid_for_step_fourth? && @character.valid_stats_assignment?
        ...
        unless @character.valid_stats_assignment?
          @errors << " Zwróć szczególna uwagę na swój dar: \"#{@character.character_background.traits.first.try(:name)}\" i sposób w jaki musisz przyporządkować drugi najwyższy wylosowany paramter."
        end
        @redirect = character_wizard_path(char_id: @character.id, step: "statistics")
      end
    else
  ...
  def fightstyle
    if params
      if (@character.fight_style.present? and @character.update_attributes(wield_style_id: params[:wield_style_id])) ||
          @character.update_attributes(fight_style_id: params[:fight_style_id], wield_style_id: params[:wield_style_id])
