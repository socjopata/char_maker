...
class Spell < ActiveRecord::Base
  ...
  scope :alchemist, lambda { fetch_for_level("1").where(:profession_id => Profession.find_by_name("Alchemik").id) } #TODO Look here in case we have more levels than 1
  ...
  scope :fetch_for_level, lambda { |level| joins(:spell_requirements).where("spell_requirements.check_applies_to = 'experience' AND  spell_requirements.value = ?", level) }
